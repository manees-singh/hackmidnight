...
export class ContractAnalyzer {
     ...
     */
    detectContractFromSource(contractSourceDir) {
     ...
     */
    parseFunctionSignatures(functionsBlock, functions, isReadOnly) {
     ...
     */
    parsePureFunctionSignatures(functionsBlock, functions) {
     ...
     */
    mapTypeScriptTypeToUserFriendly(type) {
        const typeMap = {
            'bigint': 'number',
            'Uint8Array': 'bytes',
            '[]': 'void',
    ...
    generateFunctionDescription(name, parameters) {
     ...
     */
    isReadOnlyFunction(functionName) {
        if (!this.contractAnalysis) {
            return false;
        }
        // Find the function in our analysis
        const func = this.contractAnalysis.functions.find((f) => f.name === functionName);
        if (!func) {
            return false;
        }
     ...
     */
    requiresSpecialHandling(paramName) {
